...
# pip install sentence-transformers pandas numpy scikit-learn python-dotenv langchain-google-genai google-generativeai xgboost
...
class BlogSuggestion:
    ...
    serp_score: float = 0.0  # Bonus: SERP score
...
class BlogSuggestionEngine:
    ...
    def __init__(self, dataset_path: str, db_path: str = "title_corpus.db"):
        ...
        self.load_dataset()
        self.setup_embeddings_and_database()
        self.train_serp_model()  # Bonus: Train XGBoost model for SERP score
    ...
    # def setup_embeddings_and_database(self):
    #     """Setup sentence transformer model, create embeddings, and store in SQLite"""
    ...
    #     # Create table with explicit schema including embedding as BLOB
    #     cursor.execute("""
    #         CREATE TABLE titles (
    #             title TEXT,
    #             category TEXT,
    ...
    #     print("🔄 Creating embeddings and SQLite database...")
    #     titles_list = self.titles_df['title'].tolist()
    #     embeddings = self.sentence_model.encode(titles_list, show_progress_bar=True)
    #     df_to_insert = self.titles_df.copy()
    #     df_to_insert['embedding_id'] = range(len(df_to_insert))
    #     df_to_insert['embedding'] = [pickle.dumps(emb) for emb in embeddings]
    #     for col in ['category', 'subtitle']:
    #         if col not in df_to_insert.columns:
    #             df_to_insert[col] = ''
        
    #     # Insert data into SQLite
    #     df_to_insert[['title', 'category', 'subtitle', 'embedding_id', 'embedding']].to_sql(
    #         'titles', conn, if_exists='replace', index=False, dtype={'embedding': 'BLOB'}
    #     )
    #     conn.commit()
    #     conn.close()
    ...
    def setup_embeddings_and_database(self):
        ...
        has_correct_schema = table_exists and set(['title', 'category', 'subtitle', 'embedding_id', 'embedding']).issubset(columns)
        
        if table_exists and has_correct_schema:
            # Check if table has expected number of rows
            cursor.execute("SELECT COUNT(*) FROM titles")
            row_count = cursor.fetchone()[0]
            if row_count >= len(self.titles_df):
                print("📁 Loading existing database with valid schema and sufficient data...")
                conn.close()
                return
        
        # If table is missing, has wrong schema, or insufficient data, recreate it
        print("🔄 Creating embeddings and SQLite database...")
        cursor.execute("DROP TABLE IF EXISTS titles")
        cursor.execute("""
            CREATE TABLE titles (
    ...
    def train_serp_model(self):
        """Train a simple XGBoost model for SERP score (bonus)"""
        print("🧠 Training XGBoost model for SERP score...")
        # Mock features: title length, number of stop words, has numbers (as proxies for clickability)
        stop_words = {'the', 'a', 'an', 'how', 'to', 'and', 'for', 'is', 'are', 'in', 'on'}
        X = []
        for title in self.titles_df['title']:
            length = len(title)
            stop_count = sum(1 for word in title.lower().split() if word in stop_words)
            has_number = 1 if re.search(r'\d', title) else 0
            X.append([length, stop_count, has_number])
        # Mock targets: Higher score for shorter, engaging titles (0-100 scale)
        y = [min(100, 100 - (len(t) / 2) + (10 if 'best' in t.lower() else 0)) for t in self.titles_df['title']]
        self.serp_model = XGBRegressor(objective='reg:squarederror', n_estimators=50, random_state=42)
        self.serp_model.fit(X, y)
        print("✅ XGBoost model trained.")

    def predict_serp_score(self, title: str) -> float:
        """Predict SERP score for a title using XGBoost (bonus)"""
        if not self.serp_model:
            return 0.0
        stop_words = {'the', 'a', 'an', 'how', 'to', 'and', 'for', 'is', 'are', 'in', 'on'}
        length = len(title)
        stop_count = sum(1 for word in title.lower().split() if word in stop_words)
        has_number = 1 if re.search(r'\d', title) else 0
        features = np.array([[length, stop_count, has_number]])
        return float(self.serp_model.predict(features)[0])

    def find_similar_titles(self, query_text: str, top_k: int = 10) -> List[Tuple[str, float, str]]:
        """Find similar titles using semantic similarity from SQLite embeddings"""
        if not query_text or len(query_text.strip()) < 10:
            raise ValueError("Query text is empty or too short")
        ...
        if 'embedding' not in columns:
            conn.close()
            raise ValueError("Database schema missing 'embedding' column. Re-run database setup.")
        
        cursor.execute("SELECT title, embedding, category FROM titles")
        titles, embeddings, categories = [], [], []
        for row in cursor.fetchall():
            titles.append(row[0])
            embeddings.append(pickle.loads(row[1]))
            categories.append(row[2] or 'general')
        conn.close()
        
        similarities = cosine_similarity(query_embedding, embeddings)[0]
        top_indices = np.argsort(similarities)[::-1][:top_k]
        return [(titles[idx], similarities[idx], categories[idx]) for idx in top_indices]

    def extract_keywords(self, text: str, num_keywords: int = 5) -> List[str]:
        """Extract keywords from text (improved with basic TF-IDF like frequency)"""
    ...
    def filter_stop_words(self, title: str) -> str:
        """Filter titles to not start with common stop-words"""
        stop_words = {'the', 'a', 'an', 'how', 'to', 'and', 'for', 'is', 'are', 'in', 'on'}
        words = title.split()
    ...
    def parse_llm_response(self, response_text: str) -> BlogSuggestion:
        ...
        try:
            ...
            keywords = [k.strip() for k in keywords_match.group(1).strip().split(',') if k.strip()] if keywords_match else []
            ...
            # Post-process titles to <60 chars and filter stop-words
            titles = [self.filter_stop_words(title)[:60] for title in titles]

            # Post-process meta to ~155 chars
            if len(meta_description) > 160:
                meta_description = meta_description[:157] + "..."
            elif len(meta_description) < 150:
                meta_description += " " * (150 - len(meta_description))
    ...
    def generate_with_llm(self, content: str, similar_titles: List[Tuple[str, float, str]], tone: str = None) -> BlogSuggestion:
        """Generate suggestions using the configured Google GenAI LLM."""
        title_examples = [title for title, _, _ in similar_titles[:10]]
        tone_instruction = ""
        if tone:
            if tone == 'formal':
                tone_instruction = "Use formal, professional language without slang."
            elif tone == 'casual':
                tone_instruction = "Use casual, conversational language like everyday talk."
            elif tone == 'clickbait':
                tone_instruction = "Use clickbait style with hype, questions, or surprises to grab attention."
            else:
                tone_instruction = "Use neutral tone."
        ...
        **Your Task:**
        ...
        2. **SEO meta description:**
           - This must be roughly 155 characters long.
        ...
        suggestions = self.parse_llm_response(response.content)
        # Add SERP score for the first title (bonus)
        suggestions.serp_score = self.predict_serp_score(suggestions.titles[0])
        return suggestions
    ...
    def generate_with_simple_heuristics(self, content: str, similar_titles: List[Tuple[str, float, str]]) -> BlogSuggestion:
        ...
        # Post-process titles to <60 chars and filter stop-words
        base_titles = [self.filter_stop_words(title)[:60] for title in base_titles]
        ...
        if len(first_paragraph) < 150:
            first_paragraph += " " * (150 - len(first_paragraph))
    ...
    def generate_suggestions(self, markdown_content: str, tone: str = None) -> BlogSuggestion:
        """Main function to generate blog suggestions"""
        # Input validation
        if not markdown_content or len(markdown_content.strip()) < 10:
            raise ValueError("Markdown content is empty or too short.")
        if len(markdown_content) > 100000:  # Arbitrary limit for practicality
        ...
        if self.llm:
            suggestions = self.generate_with_llm(markdown_content, similar_titles, tone)
        else:
        ...
        print(f"  🔍 SERP Score: {suggestions.serp_score}")
...
def main():
    ...
    try:
        ...
        # Example with tone (bonus)
        suggestions = engine.generate_suggestions(blog_content, tone="casual")
